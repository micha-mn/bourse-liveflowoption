...
function initiateHistoricalFlow() {
	...
	{ const menu="  <a  class='navigation nav-item nav-link active' id='nav-historicalFlow-tab'  href='/flow/historicalflow' role='tab' aria-controls='nav-historicalFlow' aria-selected='false'>Historical Flow</a>";
	  $('#nav-tab').append(menu);
	}
	  ...
	  $('#date-input').on('change', function (event) 
				 {
					    var selectedDateInput = $('#date-input').jqxDateTimeInput('getDate');
					    var currentDate = new Date(selectedDateInput);
					    var year = currentDate.getFullYear();
					    var month = String(currentDate.getMonth() + 1).padStart(2, '0');
					    var day = String(currentDate.getDate()).padStart(2, '0');
					    const selectedDate = year + '-' + month + '-' + day;
					
					    fetchDataGrid(selectedDate);
				  });
	   $("#jqxNotification").jqxNotification({  height: 40, width: "100%",appendContainer: "#notifcationContainer",  opacity: 0.9,
           animationOpenDelay: 800, autoClose:false , autoCloseDelay: 2000,  template: 'info'
      });
	  source =
	            {
		           datatype: "json",
...
function changeDate(dayOffset){
	...
	$('#date-input').jqxDateTimeInput('setDate', currentDate);
